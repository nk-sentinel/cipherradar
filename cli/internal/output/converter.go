...
var algorithmFamilyMap = map[string]cyclonedx17.AlgorithmFamily{
	...
	"chacha20-poly1305": cyclonedx17.AlgorithmFamilyChaCha20,
...
func normalizeAlgorithmFamily(internal string) cyclonedx17.AlgorithmFamily {
	...
	// Unknown family: omit rather than emit an out-of-enum value. The
	// algorithmFamily field is omitempty, so "" drops it; the component still
	// carries its name + primitive. Emitting a raw non-enum value would make the
	// whole CBOM fail CycloneDX 1.7 schema validation.
	return ""
}
...
func normalizeRelatedCryptoMaterialType(internal string) cyclonedx17.RelatedCryptoMaterialType {
	...
	// Unknown material type: fall back to the schema's "other" rather than emit
	// an out-of-enum value (which would fail CycloneDX 1.7 validation).
	return cyclonedx17.RelatedCryptoMaterialTypeOther
}
