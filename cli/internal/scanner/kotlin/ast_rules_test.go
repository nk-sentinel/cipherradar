...
// replaces the active Kotlin tables and that passing "" restores the embedded
// set. Global state is restored via t.Cleanup so other kotlin tests are
// unaffected.
func TestApplyExternalRules_RoundTrip(t *testing.T) {
	...
	yml := "version: 1\nlanguage: kotlin\n" +
		...
		"algorithm_families:\n  - { name: AES, family: aes }\n"
	if err := os.WriteFile(filepath.Join(dir, "kotlin.yml"), []byte(yml), 0o644); err != nil {
		t.Fatal(err)
	}
