...
// for simple intra-procedural constant propagation in C#.
//
// Because the scanner uses the Java tree-sitter grammar to parse C#,
// C#-specific declaration syntax (var, const, readonly) is not directly
...
// C#-specific patterns like "var algo = ...", "const string ALGO = ...",
// and "string algo = ...".
...
// plus a source-text scan for C#-specific patterns like:
//
//	var algo = "AES";
...
func (cp *ConstPropagator) CollectAssignments(node *sitter.Node, content []byte) {
	...
	// Fallback: scan source text for C#-specific patterns
	// that the Java grammar may not parse into standard variable_declarator nodes.
	cp.scanCSharpDeclarations(content)
}
...
// scanCSharpDeclarations performs a simple line-based scan of the source code
// to extract C#-specific variable declarations that the Java grammar might
// not parse into recognizable AST nodes.
func (cp *ConstPropagator) scanCSharpDeclarations(content []byte) {
	lines := strings.Split(string(content), "\n")
	for _, line := range lines {
		trimmed := strings.TrimSpace(line)
		cp.parseCSharpDecl(trimmed)
	}
}

// parseCSharpDecl tries to extract a variable binding from a C#-style
...
//	string mode = "GCM";
//	int keySize = 2048;
func (cp *ConstPropagator) parseCSharpDecl(line string) {
	// Strip trailing semicolon for easier parsing
	line = strings.TrimSuffix(line, ";")

	// Strip modifiers: const, static, readonly, private, public, protected, internal
	stripped := line
	for _, mod := range []string{"const ", "static ", "readonly ", "private ", "public ", "protected ", "internal "} {
		stripped = strings.TrimPrefix(stripped, mod)
	}

	// Must have a type and name before " = "
	eqIdx := strings.Index(stripped, " = ")
	if eqIdx < 0 {
		return
	}

	declPart := strings.TrimSpace(stripped[:eqIdx])
	valuePart := strings.TrimSpace(stripped[eqIdx+3:])

	// Split declPart into type and name: "string algo" or "var algo" or "int keySize"
	lastSpace := strings.LastIndex(declPart, " ")
	if lastSpace < 0 {
		return
	}

	namePart := strings.TrimSpace(declPart[lastSpace+1:])
