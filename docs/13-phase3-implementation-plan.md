...
1. **SonarQube:** Split into two deliverables. Phase 3 delivers `cradar scan --format sonarqube-generic` (generic issue export — 1-2 days, covers 80% of value). Full SonarQube plugin with CBOM tab deferred to Phase 4.
...
| `/container-scan-test` | Integration | Validate OCI image scanning end-to-end: pull image → extract layers → scan filesystem → produce CBOM. Tests against known container images with embedded crypto | `docker pull`, `skopeo copy`, extract layers, `cradar scan --container`, validate CBOM output | A-M3 |
...
| **A-M2** | Agent-CLIRename | A-M1 | Yes | CLI rename cbom → cradar (ADR-024), --push flag + .cradar.yml config (ADR-025). Rename binary, update GoReleaser configs, update all env vars (`CBOM_*` → `CRADAR_*`), config file (`.cbom.yml` → `.cradar.yml`), tools dir (`~/.cbom/tools/` → `~/.cradar/tools/`). Implement `--push` flag on `cradar scan`, `.cradar.yml` config file parsing, `cbom` legacy alias with deprecation warning. | `/lint`, `/test-coverage`, `/build-cross` |
| **A-M2** | **Orchestrator gate** | All A-M2 | — | 12 languages + Joern C/C++ + expanded config scanning + CLI rename verified | `/lint`, `/sec-review`, `/test-coverage`, `/fuzz`, `/build-cross` |
| **A-M3** | Agent-ContainerScanning | A-M2 | No | OCI image layer scanning: accept image reference or tar, use `go-containerregistry` (crane/gcrane) to extract filesystem layers, scan extracted files with existing 12-language scanner pipeline, merge findings per layer (attribute findings to layer SHA), produce CBOM with container metadata. `cradar scan --container <image:tag>` command. Support local tar and remote registry. | `/lint`, `/sec-review`, `/test-coverage`, `/container-scan-test` |
...
| **B-M2** | Agent-ScanUploadEndpoint | B-M1 | Yes | POST /api/v1/scans/upload endpoint (ADR-025): accept CycloneDX JSON body + project_name + group_path + branch + commit_sha. Authenticated via API key (Bearer token). Project auto-resolution: find by name within org, auto-create if user has `project:write`. Group fallback logic: group not found or no access → fall back to user's default group with identical warning (prevents group existence leakage, same pattern as GitHub 404-not-403). | `/new-api-route`, `/lint-py`, `/test-py`, `/multi-tenant-test` |
| **B-M2** | **Orchestrator gate** | All B-M2 | — | 6 compliance frameworks (NIST + FIPS + 4 new), trending, SBOM ingestion, scan upload endpoint verified | `/lint-py`, `/sec-py`, `/test-py`, `/db-validate`, `/db-seed` |
