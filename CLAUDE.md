...
**Recent (0.5.0-rc.1):** real container-image scanning — layers materialized to a temp dir and run through all 3 passes with per-layer provenance, bounded recursive archive unpacking (jar-in-jar), image config/history/labels ingested as a source, and the ELF scanner registered with static-library version fingerprinting (gh #83). Detection rules are now externally definable for every pass (`--ast-rules-dir` for Pass 1, `--yara-rules-dir` for Pass 3, mirroring `--rules-dir`; gh #114). Scan memory bounded — the walker streams file reads (peak ~1.49 GB → ~224 MB on a 1.5 GB input) with `--max-file-size`; added `--max-image-size` and `--archive-max-depth` coverage knobs. Noisy entropy rules (`cbom-regex-hex-key`/`base64-key`) off by default (gh #70); `--asset-type`/`--exclude-type` CBOM filters (gh #49); Pass-3 tempdir leak fixed (gh #82). Tooling upgraded to OpenGrep v1.29.0 + YARA-X v1.20.0. See CHANGELOG.md.

**Earlier (v0.4.0-rc.4):** keystore format coverage — JCEKS + BKS parsed via pure-Go readers, BCFKS/UBER/macOS Keychain/Mozilla NSS captured presence-only, PKCS12 `.pkcs12`/`.pk12` variants, config/source keystore-password harvesting (coverage-only); richer certificate metadata (`cradar:cert:*`) + unparsed-cert surfacing (`cbom-cert-unparsed`).
